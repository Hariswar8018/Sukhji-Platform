...
class UpdateQuizId extends StatefulWidget {
  QuizTypeModel quiz;
  UpdateQuizId({super.key,required this.quiz});

  @override
  State<UpdateQuizId> createState() => _UpdateQuizIdState();
}

class _UpdateQuizIdState extends State<UpdateQuizId> {
  ...
  void initState(){
    s=widget.quiz.dateTime;
    names.text=widget.quiz.qName;
    winning.text=widget.quiz.winning;
    price.text=widget.quiz.price.toString();
    capacity.text=widget.quiz.spots.toString();
    co.text=widget.quiz.timeName;
    ordernumber=widget.quiz.orderNumber;
    name1=widget.quiz.orderNumber+" ${iup0(widget.quiz.orderNumber)}";
    print(name1);
    name.text=widget.quiz.description;
    instructions.text=widget.quiz.instruction;
    hj=widget.quiz.marks;
  }
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Update Quiz',style: TextStyle(color: Colors.white),
        ),
      ),
            ...
            onTap: () async {
              try {
                ...
                QuizTypeModel quizz = QuizTypeModel(
                    ...
                    qName: names.text, winning: winning.text, questions: widget.quiz.questions, spots: widget.quiz.spots, registered: widget.quiz.registered, price: pri);
                await FirebaseFirestore.instance.collection("Quiz").doc(widget.quiz.id).update(quizz.toJson());
